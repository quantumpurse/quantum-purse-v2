...
pub fn categorize_dao_cells(
    ...
    let cells = collect_dao_cells(ckb_client, address)?;
    ...
    for cell in cells {
        ...
        if cell_data == 0 {
            ...
            let (max_withdraw, deposit_block_number, prepare_block_number) =
                calculate_max_withdraw(ckb_client, &cell)?;
...
fn collect_dao_cells(
    ...
    ckb_client.collect_cells(&query)
}
...
fn calculate_max_withdraw(
    ...
    let prepare_tx_status = ckb_client
    ...
    let deposit_tx_status = ckb_client
