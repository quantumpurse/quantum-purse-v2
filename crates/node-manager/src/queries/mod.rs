...
//!
//! Every function here takes a `&dyn CkbRpc` and returns domain data —
...
//!
//! - the `rpc` module (the `CkbRpc` trait and its concrete clients), and
//! - the `tx_builder` module (which constructs transactions).
//!
//! The `NodeManager` facade in `lib.rs` exposes these as methods so callers
...
pub use dao_cells::{categozire_dao_cells, DepositedCell, PreparedCell};
