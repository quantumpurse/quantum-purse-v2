...
impl App {
			...
			NodeType::LightClient => {
				match (self.node_status.synced_block, self.node_status.tip_block) {
					(Some(s), Some(t)) if t > 0 => {
						(s as f64 / t as f64).clamp(0.0, 1.0) as f32
					}
					_ => 0.0,
				}
			}
			NodeType::FullNode => {
				full_node_sync_view(self.node_status.sync_state.as_ref()).0
			}
	...
	fn draw_backend_section(&mut self, ui: &mut egui::Ui, backend: NodeType) {
		...
		if !active {
			let click = ui
				.interact(header_rect, ui.id().with(backend as u8), egui::Sense::click())
				.on_hover_cursor(egui::CursorIcon::PointingHand);
			if click.hovered() {
		...
		let pct = if has_sync { self.sync_pct(backend) } else { 0.0 };
